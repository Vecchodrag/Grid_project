...
void info_menu::show_info(const std::string& info, int y)const {
    int i=0,k=y;
    std::string sub_info;
    while(i<info.size()) {
...
void info_menu::clean() const{
    int ymax=55;
    for(int i=1;i<ymax/2-1;i++)
        mvwprintw(info_window,i,1,"                                      ");
}

void info_menu::display_cell_info(const std::shared_ptr<Cell>& cell) const{
